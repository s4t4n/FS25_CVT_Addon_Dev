...
if g_specializationManager:getSpecializationByName("CVTaddon") == nil then
	...
	if g_currentModName ~= "FS25_CVT_Addon" then
		if string.find(tostring(g_currentModName), "update") then
			print("CVTaddon: Update Version in use. Please rename without _update prefix.")
		elseif string.find(tostring(g_currentModName), "master") or string.find(tostring(g_currentModName), "main") then
