...
cvtaDebugCVTon = true	 -- \
debug_for_DBL = false	  --  \ 
cvtaDebugCVTxOn = true	  --   } Debug change via console commands
    ...
    if intVehicles or int2ndVehicles then
        ...
        if isVario == true and integrateConfig == true and not exVehicles then
            ...
            configurations["CVTaddon"] = {
                {name = name1, index = 1, isDefault = true, price = 0, dailyUpkeep = 0, isSelectable = true},
					...
					if spec.CVTconfig ~= 7 then
						if spec.vTwo == 4 and spec.isVarioTM then
							if self:getLastSpeed() <= (self.spec_motorized.motor.maxForwardSpeed*math.pi)/(2.6) then -- Beschleunigung wird ab kmh X full
								self.spec_motorized.motor.accelerationLimit = 1.7 * math.max((1-spec.ClutchInputValue), 0.01) -- Standard IV
							else
								self.spec_motorized.motor.accelerationLimit = 1.8 * math.max((1-spec.ClutchInputValue), 0.01) -- Standard
							end
							...
							if (self:getTotalMass() - self:getTotalMass(true)) ~= 0 then -- 35 100
								...
								self.spec_motorized.motor.lowBrakeForceScale = ( math.max(math.min((0.5-((self:getTotalMass() - self:getTotalMass(true)) /100 ))*(0.8-(self:getLastSpeed()/100)), 0.35*( 1-(self:getTotalMass() - self:getTotalMass(true))/100 ) ), 0.01) )*(1-spec.ClutchInputValue)
								...
								self.spec_motorized.motor.lowBrakeForceScale = ( math.max(math.min((1-(self.spec_motorized.motor.lastMotorRpm/((self.spec_motorized.motor.minRpm/2)*10))-(self:getLastSpeed()/100)),0.35),0.01) )*(1-spec.ClutchInputValue)
							end
						...
						if spec.vTwo == 1 and spec.isVarioTM then
							if self:getLastSpeed() <= (self.spec_motorized.motor.maxForwardSpeed*math.pi)/(2.6) then
								self.spec_motorized.motor.accelerationLimit = 0.35 * math.max((1-spec.ClutchInputValue), 0.01) -- I
							else
							...
							if (self:getTotalMass() - self:getTotalMass(true)) ~= 0 then -- 20 97
								self.spec_motorized.motor.lowBrakeForceScale = ( math.max(math.min((0.5-((self:getTotalMass() - self:getTotalMass(true)) /97 ))*(0.8-(self:getLastSpeed()/100)), 0.2*( 1-(self:getTotalMass() - self:getTotalMass(true))/100 ) ), 0.01) )*(1-spec.ClutchInputValue)
							else
								self.spec_motorized.motor.lowBrakeForceScale = ( math.max(math.min((1-(self.spec_motorized.motor.lastMotorRpm/((self.spec_motorized.motor.minRpm/2.03)*10))-(self:getLastSpeed()/100)),0.2),0.01) )*(1-spec.ClutchInputValue)
							end
							-- Sprit-Verbrauch anpassen
						...
						if spec.vTwo == 2 and spec.isVarioTM then
							if self:getLastSpeed() <= (self.spec_motorized.motor.maxForwardSpeed*math.pi)/(2.6) then
								self.spec_motorized.motor.accelerationLimit = 0.80 * math.max((1-spec.ClutchInputValue), 0.01) -- II
							else
							...
							if (self:getTotalMass() - self:getTotalMass(true)) ~= 0 then -- 25 98
								self.spec_motorized.motor.lowBrakeForceScale = ( math.max(math.min((0.5-((self:getTotalMass() - self:getTotalMass(true)) /98 ))*(0.8-(self:getLastSpeed()/100)), 0.25*( 1-(self:getTotalMass() - self:getTotalMass(true))/100 ) ), 0.01) )*(1-spec.ClutchInputValue)
							else
								self.spec_motorized.motor.lowBrakeForceScale = ( math.max(math.min((1-(self.spec_motorized.motor.lastMotorRpm/((self.spec_motorized.motor.minRpm/2.02)*10))-(self:getLastSpeed()/100)),0.25),0.01) )*(1-spec.ClutchInputValue)
							end
							-- Sprit-Verbrauch anpassen
						...
						if spec.vTwo == 3 and spec.isVarioTM then
							if self:getLastSpeed() <= (self.spec_motorized.motor.maxForwardSpeed*math.pi)/(2.6) then
								self.spec_motorized.motor.accelerationLimit = 1.20 * math.max((1-spec.ClutchInputValue), 0.01) -- III
							else
							...
							if (self:getTotalMass() - self:getTotalMass(true)) ~= 0 then -- 30 99
								self.spec_motorized.motor.lowBrakeForceScale = ( math.max(math.min((0.5-((self:getTotalMass() - self:getTotalMass(true)) /99 ))*(0.8-(self:getLastSpeed()/100)), 0.30*( 1-(self:getTotalMass() - self:getTotalMass(true))/100 ) ), 0.01) )*(1-spec.ClutchInputValue)
							else
								self.spec_motorized.motor.lowBrakeForceScale = ( math.max(math.min((1-(self.spec_motorized.motor.lastMotorRpm/((self.spec_motorized.motor.minRpm/2.01)*10))-(self:getLastSpeed()/100)),0.3),0.01) )*(1-spec.ClutchInputValue)
							end
							-- Sprit-Verbrauch anpassen
